...
module.exports = async (req, res) => {
  ...
  if (action === 'telegram-unlink') {
    ...
    if (row?.telegram_chat_id && config.BOT_URL && config.WEBHOOK_SECRET) {
      log('info', 'auth.unlink_invalidating', { userId: user.id });
      try {
      ...
      log('info', 'auth.unlink_skip', { userId: user.id, hasChatId: !!row?.telegram_chat_id, hasBotUrl: !!config.BOT_URL, hasWebhookSecret: !!config.WEBHOOK_SECRET });
    }
